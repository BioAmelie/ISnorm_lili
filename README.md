...
The parameter `switch_check` specifies the parameter used for reliability check. By `switch_check=2`, ISnorm will examine the next two candidate geneset after the chosen geneset. If they share no common genes, ISnorm will print a warning message (in this case the results of ISnorm is sensitive to the value of `baseline_threshold`, see our article for more details). But this parameter won't change the results.<br><br>
